function constructDistancedSequence(n: number): number[] {
    const resultSequence = Array.from({length: n * 2 - 1}, () => 0);
...
function findLexicographicallyLargestSequence(currentIndex: number, resultSequence: number[], isNumberUsed: boolean[], targetNumber: number) {
